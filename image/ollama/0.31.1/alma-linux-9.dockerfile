...
RUN set -x \
  && dnf install -y zstd \
  && wget https://github.com/ollama/ollama/releases/download/v0.31.1/ollama-linux-amd64.tar.zst --quiet \
  && tar xf ollama-linux-amd64.tar.zst > /dev/null \
...
RUN set -x \
  && dnf install -y zstd \
  && wget https://github.com/ollama/ollama/releases/download/v0.31.1/ollama-linux-arm64.tar.zst --quiet \
  && tar xf ollama-linux-arm64.tar.zst > /dev/null \
  && mkdir /usr/local/lib/ollama \
  && cp -r lib/ollama/* /usr/local/lib/ollama/ \
  && mv bin/ollama /usr/local/bin/ \
  && rm -rf ollama* lib bin \
